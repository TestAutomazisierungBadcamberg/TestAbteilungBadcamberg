...
public class GeCBewerberSortierung extends BaseClass {
    ...
    public void ichSortiereJeNachSpalte() {
        ...
        sleep(1000);
        String text = driver.findElements(By.xpath(PropertyReaders.read("GeC_Bewerber").get("BewerberListeNamenAllListe"))).get(6).getText();
        click(elements.get(3));
        click(bewerber.fußLeisteButtons("Ok"));
        sleep(1000);
        String text2 = driver.findElements(By.xpath(PropertyReaders.read("GeC_Bewerber").get("BewerberListeNamenAllListe"))).get(6).getText();
